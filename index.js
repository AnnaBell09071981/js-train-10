...
function checkProperty(obj, prop) {
  ...
  const hasOwnPropertyObj = obj.hasOwnProperty(prop);
  return hasOwnPropertyObj;
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const freezeObj = Object.freeze(newObject);
  const fObj = Object.isFrozen(freezeObj);
  if(fObj === true) {
    return freezeObj;
  } else {
    return false;
...
function checkAndPreventExtensions(obj) {
  ...
  const newExt = Object.isExtensible(obj);
  if(Object.isExtensible(obj) === true) {
      const newPrev = Object.preventExtensions(obj);
  if(Object.isExtensible(obj) === true) {
      return true;
  } else {
    return false;
...
function sealAndCheck(obj) {
  ...
  const objIsSeal = Object.isSealed(obj);
  return objIsSeal;
  // Перевіряємо, чи вхідний параметр є об'єктом, якщо ні, повертаємо false
...
function checkOwnership(obj1, obj2, key) {
  const obj1Has = obj1.hasOwnProperty(key);
  const obj2Has = obj2.hasOwnProperty(key);
  return Object.is(obj1Has, obj2Has);
